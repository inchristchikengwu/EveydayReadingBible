const STORAGE_KEYS = {
  endpoint: 'dailyDevotion.endpoint',
  token: 'dailyDevotion.token',
  fontSize: 'dailyDevotion.fontSize',
  lastPayload: 'dailyDevotion.lastPayload',
  theme: 'dailyDevotion.theme',
  indexPayload: 'dailyDevotion.indexPayload',
  indexFetchedAt: 'dailyDevotion.indexFetchedAt'
};

const SECTION_COLORS = [
  '#2a6f82', '#6f4e9b', '#287a57', '#b2602a', '#703775', '#1f6a96', '#914c2f', '#42733f'
];

const SECTION_SYMBOLS = ['◆', '✦', '✧', '▣', '❖', '◇', '●', '✺'];

const $ = (id) => document.getElementById(id);

const els = {
  settingsToggle: $('settingsToggle'),
  fontToggle: $('fontToggle'),
  dateToggle: $('dateToggle'),
  indexToggle: $('indexToggle'),
  settingsPanel: $('settingsPanel'),
  fontPanel: $('fontPanel'),
  datePanel: $('datePanel'),
  indexPanel: $('indexPanel'),
  endpointInput: $('endpointInput'),
  tokenInput: $('tokenInput'),
  saveSettingsBtn: $('saveSettingsBtn'),
  clearSettingsBtn: $('clearSettingsBtn'),
  settingsStatus: $('settingsStatus'),
  lightThemeBtn: $('lightThemeBtn'),
  darkThemeBtn: $('darkThemeBtn'),
  dateInput: $('dateInput'),
  loadBtn: $('loadBtn'),
  inlineStatus: $('inlineStatus'),
  refreshIndexBtn: $('refreshIndexBtn'),
  indexSearchInput: $('indexSearchInput'),
  indexStatus: $('indexStatus'),
  indexList: $('indexList'),
  fontDownBtn: $('fontDownBtn'),
  fontUpBtn: $('fontUpBtn'),
  fontResetBtn: $('fontResetBtn'),
  metaPanel: $('metaPanel'),
  devotionTitle: $('devotionTitle'),
  sectionNav: $('sectionNav'),
  reader: $('reader')
};

let state = {
  payload: null,
  activeSection: 'all',
  fontSize: Number(localStorage.getItem(STORAGE_KEYS.fontSize) || 18),
  theme: localStorage.getItem(STORAGE_KEYS.theme) || 'light',
  indexItems: [],
  indexLoading: false
};

init();

function init() {
  els.dateInput.value = taipeiDateString(new Date());
  els.endpointInput.value = localStorage.getItem(STORAGE_KEYS.endpoint) || '';
  els.tokenInput.value = localStorage.getItem(STORAGE_KEYS.token) || '';
  applyFontSize(state.fontSize);
  applyTheme(state.theme);
  wireEvents();
  restoreCachedPayload();

  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('./sw.js').catch(() => {});
  }
}

function wireEvents() {
  els.settingsToggle.addEventListener('click', () => togglePanel('settings'));
  els.fontToggle.addEventListener('click', () => togglePanel('font'));
  els.dateToggle.addEventListener('click', () => togglePanel('date'));
  els.indexToggle.addEventListener('click', async () => {
    const opened = togglePanel('index');
    if (opened) await loadIndex();
  });

  els.lightThemeBtn.addEventListener('click', () => applyTheme('light'));
  els.darkThemeBtn.addEventListener('click', () => applyTheme('dark'));

  els.saveSettingsBtn.addEventListener('click', () => {
    const endpoint = els.endpointInput.value.trim();
    const token = els.tokenInput.value.trim();
    if (!endpoint) {
      setText(els.settingsStatus, '請先貼上 Apps Script Web App URL。', true);
      return;
    }
    localStorage.setItem(STORAGE_KEYS.endpoint, endpoint);
    localStorage.setItem(STORAGE_KEYS.token, token);
    localStorage.removeItem(STORAGE_KEYS.indexPayload);
    localStorage.removeItem(STORAGE_KEYS.indexFetchedAt);
    setText(els.settingsStatus, '設定已儲存。');
    setInlineStatus('已儲存');
  });

  els.clearSettingsBtn.addEventListener('click', () => {
    localStorage.removeItem(STORAGE_KEYS.endpoint);
    localStorage.removeItem(STORAGE_KEYS.token);
    localStorage.removeItem(STORAGE_KEYS.indexPayload);
    localStorage.removeItem(STORAGE_KEYS.indexFetchedAt);
    state.indexItems = [];
    els.indexList.innerHTML = '';
    els.endpointInput.value = '';
    els.tokenInput.value = '';
    setText(els.settingsStatus, '設定已清除。');
    setInlineStatus('未設定');
  });

  els.loadBtn.addEventListener('click', () => loadDevotion());
  els.refreshIndexBtn.addEventListener('click', () => loadIndex(true));
  els.indexSearchInput.addEventListener('input', renderIndexList);
  els.fontDownBtn.addEventListener('click', () => changeFontSize(-1));
  els.fontUpBtn.addEventListener('click', () => changeFontSize(1));
  els.fontResetBtn.addEventListener('click', () => {
    state.fontSize = 18;
    applyFontSize(state.fontSize);
  });
}

function togglePanel(panelName) {
  const panelMap = {
    settings: [els.settingsPanel, els.settingsToggle],
    font: [els.fontPanel, els.fontToggle],
    date: [els.datePanel, els.dateToggle],
    index: [els.indexPanel, els.indexToggle]
  };

  const [targetPanel, targetButton] = panelMap[panelName];
  const willOpen = targetPanel.classList.contains('hidden');

  Object.values(panelMap).forEach(([panel, button]) => {
    panel.classList.add('hidden');
    button.classList.remove('active');
    button.setAttribute('aria-expanded', 'false');
  });

  if (willOpen) {
    targetPanel.classList.remove('hidden');
    targetButton.classList.add('active');
    targetButton.setAttribute('aria-expanded', 'true');
  }

  return willOpen;
}

function taipeiDateString(date) {
  const parts = new Intl.DateTimeFormat('en-CA', {
    timeZone: 'Asia/Taipei',
    year: 'numeric',
    month: '2-digit',
    day: '2-digit'
  }).formatToParts(date);

  const map = Object.fromEntries(parts.map((part) => [part.type, part.value]));
  return `${map.year}-${map.month}-${map.day}`;
}

function setText(el, message, isError = false) {
  el.textContent = message;
  el.style.color = isError ? '#d85c4a' : '';
}

function setInlineStatus(message, isError = false) {
  els.inlineStatus.textContent = message;
  els.inlineStatus.style.color = isError ? '#ffd1ca' : '';
}

function changeFontSize(delta) {
  state.fontSize = Math.min(30, Math.max(15, state.fontSize + delta));
  applyFontSize(state.fontSize);
}

function applyFontSize(size) {
  document.documentElement.style.setProperty('--reader-size', `${size}px`);
  localStorage.setItem(STORAGE_KEYS.fontSize, String(size));
}

function applyTheme(theme) {
  state.theme = theme === 'dark' ? 'dark' : 'light';
  document.documentElement.dataset.theme = state.theme;
  document.querySelector('meta[name="theme-color"]')?.setAttribute(
    'content',
    state.theme === 'dark' ? '#0e1513' : '#2f5d50'
  );

  els.lightThemeBtn?.classList.toggle('active', state.theme === 'light');
  els.darkThemeBtn?.classList.toggle('active', state.theme === 'dark');
  localStorage.setItem(STORAGE_KEYS.theme, state.theme);
}

function restoreCachedPayload() {
  const cached = localStorage.getItem(STORAGE_KEYS.lastPayload);
  if (!cached) {
    setInlineStatus(localStorage.getItem(STORAGE_KEYS.endpoint) ? '待讀取' : '未設定');
    return;
  }

  try {
    const payload = JSON.parse(cached);
    if (payload?.ok) {
      state.payload = payload;
      renderPayload(payload, true);
      setInlineStatus('快取');
    }
  } catch {
    localStorage.removeItem(STORAGE_KEYS.lastPayload);
    setInlineStatus('待讀取');
  }
}

function readCachedIndex() {
  const raw = localStorage.getItem(STORAGE_KEYS.indexPayload);
  const fetchedAt = Number(localStorage.getItem(STORAGE_KEYS.indexFetchedAt) || 0);
  if (!raw) return null;

  try {
    const payload = JSON.parse(raw);
    if (!payload?.ok || !Array.isArray(payload.items)) return null;
    return { payload, fetchedAt };
  } catch {
    localStorage.removeItem(STORAGE_KEYS.indexPayload);
    localStorage.removeItem(STORAGE_KEYS.indexFetchedAt);
    return null;
  }
}

async function loadIndex(force = false) {
  if (state.indexLoading) return;

  const endpoint = localStorage.getItem(STORAGE_KEYS.endpoint) || '';
  const token = localStorage.getItem(STORAGE_KEYS.token) || '';
  if (!endpoint) {
    togglePanel('settings');
    setText(els.settingsStatus, '請先貼上 Google Apps Script Web App URL。', true);
    return;
  }

  const cached = readCachedIndex();
  const cacheFresh = cached && Date.now() - cached.fetchedAt < 15 * 60 * 1000;
  if (!force && cacheFresh) {
    state.indexItems = cached.payload.items;
    renderIndexList();
    setText(els.indexStatus, `共 ${state.indexItems.length} 篇靈修（本機快取）`);
    return;
  }

  state.indexLoading = true;
  els.refreshIndexBtn.disabled = true;
  setText(els.indexStatus, '正在讀取日期與主題索引…');
  els.indexList.innerHTML = '<div class="index-empty">正在整理靈修資料庫…</div>';

  try {
    const payload = await jsonpRequest(endpoint, {
      action: 'index',
      token,
      refresh: force ? '1' : '0'
    });

    if (!payload.ok || !Array.isArray(payload.items)) {
      throw new Error(payload.message || payload.error || '索引讀取失敗');
    }

    state.indexItems = payload.items;
    localStorage.setItem(STORAGE_KEYS.indexPayload, JSON.stringify(payload));
    localStorage.setItem(STORAGE_KEYS.indexFetchedAt, String(Date.now()));
    renderIndexList();
    setText(els.indexStatus, `共 ${state.indexItems.length} 篇靈修，按日期由新到舊排列。`);
  } catch (error) {
    if (cached?.payload?.items?.length) {
      state.indexItems = cached.payload.items;
      renderIndexList();
      setText(els.indexStatus, `無法更新，暫時顯示 ${state.indexItems.length} 篇本機快取。`, true);
    } else {
      els.indexList.innerHTML = '<div class="index-empty">索引讀取失敗，請檢查 Apps Script 是否已更新並重新部署。</div>';
      setText(els.indexStatus, friendlyError(error), true);
    }
  } finally {
    state.indexLoading = false;
    els.refreshIndexBtn.disabled = false;
  }
}

function renderIndexList() {
  const query = (els.indexSearchInput.value || '').trim().toLowerCase();
  const items = state.indexItems.filter((item) => {
    const searchable = `${item.date || ''} ${item.topic || ''} ${item.title || ''}`.toLowerCase();
    return !query || searchable.includes(query);
  });

  els.indexList.innerHTML = '';
  if (!items.length) {
    els.indexList.innerHTML = `<div class="index-empty">${query ? '找不到符合條件的日期或主題。' : '目前沒有可顯示的靈修文件。'}</div>`;
    return;
  }

  items.forEach((item) => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'index-item';
    button.setAttribute('aria-label', `讀取 ${item.date} ${item.topic || '每日靈修'}`);

    const date = document.createElement('span');
    date.className = 'index-date';
    date.textContent = item.date || '日期未明';

    const topic = document.createElement('span');
    topic.className = 'index-topic';
    topic.textContent = item.topic || item.title || '每日靈修';

    const arrow = document.createElement('span');
    arrow.className = 'index-arrow';
    arrow.setAttribute('aria-hidden', 'true');
    arrow.textContent = '→';

    button.append(date, topic, arrow);
    button.addEventListener('click', async () => {
      togglePanel('index');
      await loadDevotion(item.date);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    });

    els.indexList.appendChild(button);
  });
}

async function loadDevotion(requestedDate) {
  const endpoint = localStorage.getItem(STORAGE_KEYS.endpoint) || '';
  const token = localStorage.getItem(STORAGE_KEYS.token) || '';
  const date = requestedDate || els.dateInput.value || taipeiDateString(new Date());
  els.dateInput.value = date;

  if (!endpoint) {
    togglePanel('settings');
    setInlineStatus('未設定', true);
    setText(els.settingsStatus, '請先貼上 Google Apps Script Web App URL。', true);
    return;
  }

  setInlineStatus('讀取中');
  els.loadBtn.disabled = true;

  try {
    const payload = await jsonpRequest(endpoint, { date, token });
    if (!payload.ok) {
      throw new Error(payload.message || payload.error || '讀取失敗');
    }

    state.payload = payload;
    state.activeSection = 'all';
    localStorage.setItem(STORAGE_KEYS.lastPayload, JSON.stringify(payload));
    renderPayload(payload, false);
    setInlineStatus('完成');

    setTimeout(() => {
      if (els.inlineStatus.textContent === '完成') setInlineStatus('已載入');
    }, 1800);
  } catch (error) {
    setInlineStatus('失敗', true);
    showReaderError(friendlyError(error));
  } finally {
    els.loadBtn.disabled = false;
  }
}

function jsonpRequest(baseUrl, params) {
  return new Promise((resolve, reject) => {
    const callbackName = `dailyDevotionCallback_${Date.now()}_${Math.floor(Math.random() * 100000)}`;
    const url = new URL(baseUrl);
    Object.entries(params).forEach(([key, value]) => {
      if (value !== undefined && value !== null) url.searchParams.set(key, value);
    });
    url.searchParams.set('callback', callbackName);

    const script = document.createElement('script');
    const cleanup = () => {
      delete window[callbackName];
      script.remove();
      clearTimeout(timer);
    };

    const timer = setTimeout(() => {
      cleanup();
      reject(new Error('連線逾時，請確認 Apps Script URL 與部署權限。'));
    }, 20000);

    window[callbackName] = (data) => {
      cleanup();
      resolve(data);
    };

    script.onerror = () => {
      cleanup();
      reject(new Error('無法連線到 Apps Script。請確認 Web App URL 是否正確。'));
    };

    script.src = url.toString();
    document.head.appendChild(script);
  });
}

function friendlyError(error) {
  const msg = String(error?.message || error);
  if (msg.includes('UNAUTHORIZED')) return 'Token 不正確，請檢查設定中的讀取 Token。';
  if (msg.includes('NOT_FOUND')) return '找不到當日靈修。請確認標題格式為「每日靈修 YYYY-MM-DD｜主題」，且文件在「每日靈修」資料夾內。';
  return msg;
}

function showReaderError(message) {
  els.reader.innerHTML = `
    <div class="empty-state">
      <div class="cross">!</div>
      <h2>讀取失敗</h2>
      <p>${escapeHtml(message)}</p>
    </div>
  `;
}

function renderPayload(payload, fromCache) {
  els.metaPanel.classList.remove('hidden');
  els.devotionTitle.textContent = payload.title || `每日靈修 ${payload.date || ''}`;

  renderSectionNav(payload.sections || []);
  renderReader(payload.sections || []);
}

function renderSectionNav(sections) {
  els.sectionNav.innerHTML = '';
  if (!sections.length) {
    els.sectionNav.classList.add('hidden');
    return;
  }

  const allBtn = makeSectionButton('全部', 'all');
  els.sectionNav.appendChild(allBtn);

  sections.forEach((section) => {
    const btn = makeSectionButton(section.shortTitle || section.title, section.id);
    els.sectionNav.appendChild(btn);
  });

  els.sectionNav.classList.remove('hidden');
}

function makeSectionButton(label, id) {
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = `section-chip ${state.activeSection === id ? 'active' : ''}`;
  btn.textContent = label;
  btn.addEventListener('click', () => {
    state.activeSection = id;
    document.querySelectorAll('.section-chip').forEach((chip) => chip.classList.remove('active'));
    btn.classList.add('active');
    renderReader(state.payload?.sections || []);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  });
  return btn;
}

function renderReader(sections) {
  els.reader.innerHTML = '';

  const visible = state.activeSection === 'all'
    ? sections
    : sections.filter((section) => section.id === state.activeSection);

  if (!visible.length) {
    els.reader.innerHTML = `<div class="empty-state"><h2>此單元沒有內容</h2><p>請選擇其他單元或重新讀取。</p></div>`;
    return;
  }

  visible.forEach((section, index) => {
    const sectionEl = document.createElement('section');
    sectionEl.className = 'devotion-section';
    sectionEl.style.setProperty('--section-color', section.color || SECTION_COLORS[index % SECTION_COLORS.length]);

    const heading = document.createElement('h2');
    const symbol = document.createElement('span');
    symbol.className = 'section-symbol';
    symbol.textContent = section.symbol || SECTION_SYMBOLS[index % SECTION_SYMBOLS.length];
    heading.appendChild(symbol);
    heading.appendChild(document.createTextNode(section.title));
    sectionEl.appendChild(heading);

    (section.lines || []).forEach((line) => {
      const p = document.createElement('p');
      p.className = classifyLine(line);
      if (isEnglishLine(line)) p.lang = 'en';
      appendTextWithLinks(p, line);
      sectionEl.appendChild(p);
    });

    els.reader.appendChild(sectionEl);
  });
}

function isEnglishLine(line) {
  const text = String(line || '').replace(/https?:\/\/\S+/g, '');
  const latinCount = (text.match(/[A-Za-z]/g) || []).length;
  const cjkCount = (text.match(/[\u3400-\u9fff]/g) || []).length;
  return latinCount >= 3 && (cjkCount === 0 || latinCount >= cjkCount * 2.5);
}

function classifyLine(line) {
  const classes = ['devotion-line'];
  if (isEnglishLine(line)) classes.push('english');
  if (/^(中文|English|簡介|Introduction|與今日經文的關聯|Relation to Today|YouTube|Key sentence|中文歌名|常見英文歌名)[：:]?$/.test(line.trim())) classes.push('subhead');
  if (/^「.*」$|^“.*”$/.test(line.trim())) classes.push('quote');
  if (/安靜，不是|Stillness does not|我不必等風浪|I do not need/.test(line)) classes.push('key');
  return classes.join(' ');
}

function appendTextWithLinks(element, text) {
  const urlPattern = /(https?:\/\/[^\s]+)/g;
  let lastIndex = 0;
  let match;

  while ((match = urlPattern.exec(text)) !== null) {
    if (match.index > lastIndex) {
      element.appendChild(document.createTextNode(text.slice(lastIndex, match.index)));
    }

    const a = document.createElement('a');
    a.href = match[0];
    a.textContent = match[0];
    a.target = '_blank';
    a.rel = 'noopener';
    a.className = 'devotion-link';
    element.appendChild(a);

    lastIndex = match.index + match[0].length;
  }

  if (lastIndex < text.length) {
    element.appendChild(document.createTextNode(text.slice(lastIndex)));
  }
}

function escapeHtml(text) {
  return String(text).replace(/[&<>"']/g, (char) => ({
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;',
    '"': '&quot;',
    "'": '&#039;'
  }[char]));
}
