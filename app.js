...
async function loadDevotion() {
  ...
  try {
    ...
    if (!payload.ok) throw new Error(payload.message || payload.error || '讀取失敗');
    state.payload = payload;
...
function renderPayload(payload, fromCache) {
  ...
  els.devotionDate.textContent = `${payload.date}${fromCache ? '｜快取' : ''}`;
...
function renderSectionNav(sections) {
  ...
  els.sectionNav.appendChild(makeSectionButton('全部', 'all'));
  sections.forEach((section) => {
    els.sectionNav.appendChild(makeSectionButton(section.shortTitle || section.title, section.id));
  });
