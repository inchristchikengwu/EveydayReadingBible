 ...
 * 2. 將本檔貼到 Code.gs。
 * 3. 設定 FOLDER_ID 與 APP_TOKEN。
 * 4. Deploy > New deployment > Web app。
 *    - Execute as: Me
 *    - Who has access: Anyone
 * 5. 將 Web app URL 貼到 PWA 的「設定」。
 *
 * 安全提醒：
 * - APP_TOKEN 不要放到 GitHub repo。
 * - PWA 會請你在 iPhone 上輸入 token，並只存於 localStorage。
 */

const FOLDER_ID = '1OJO_yM_pBxaXqFMdpoBE_2lwlOwp0sD5'; // 你的 Google Drive「每日靈修」資料夾 ID
const APP_TOKEN = 'CHANGE_ME_TO_A_LONG_RANDOM_TOKEN';    // 請改成一串夠長的自訂密碼
...
function readDevotionByDate_(date) {
  ...
  while (files.hasNext()) {
    ...
    if (file.getMimeType() === MimeType.GOOGLE_DOCS && file.getName().indexOf(titlePrefix) === 0) {
  ...
  if (!target) {
    return {
      ...
      error: 'NOT_FOUND',
      date,
      message: `找不到標題開頭為「${titlePrefix}」的 Google Docs。`
    };
  }
...
function splitSections_(text) {
  ...
  lines.forEach((line) => {
    ...
    if (isSectionHeading_(trimmed)) {
      ...
      else if (introLines.some((item) => item.trim())) {
        sections.push(makeSection_('intro', '導言｜Introduction', introLines));
      }
  ...
  if (current) sections.push(current);
    ...
    sections.push(makeSection_('content', '全文｜Full Text', introLines));
  }
